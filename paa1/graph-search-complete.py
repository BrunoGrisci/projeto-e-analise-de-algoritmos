...
## Grafos/dígrafos como dicionários
...
## Registro global de nodos visitados pela busca
visited = {} 
for i in g1.keys():
	visited[i]=False
...
def bfsIter(g,v):   # g é um grafo, v é um nodo
	visited = {} 
	for i in g.keys():
		visited[i]=False   # lista de visitados
		
	l = [v];               # inicia fila unitária com v
	visited[v]=True        # registra que h foi visitado
	
	while len(l)>0:     # enquanto há elementos em l
		h = l.pop(0)    # extrai em h a cabeça fila
		print(h)        # faça algo com h
		for i in g[h]:          # para vizinhos de h
			if not(visited[i]): # se vizinho não visitado
...
def dfsIterTree(g,v):   # g é um grafo, v é um nodo
...
def distance(g,v):   # g é um grafo, v é um nodo
...
def hasCycle(g):
	...
	for x in g.keys():
		if not visited[x]:
			l = [x];           # inicia lista unitária com x
			...
			while len(l)>0:    # enquanto há elementos em l
				...
				for i in g[h]:          # para todos os vizinhos de h
					if not(visited[i]): # i é vizinho não visitado
						l.append(i)         # insere na cabeça da lista
						parent[i]  = h      # marca h como pai de i
						visited[i] = True   # marca como visitado
					elif i != parent[h]: # i é vizinho visitado (e distinto do pai de h)
	...
	for x in g.keys():         # escolhe um nodo x por componente
		if not(visited[x]):    # se nao visitado	   			
			...
			while len(l)>0:      # enquanto a nodos na busca
				i = l.pop(0)     # remove o nodo da frente
				for j in g[i]:   # para todos os seus vizinhos
					if visited[j] and cor[i]==cor[j]:  # se vizinho já colorido com cor oposta
...
def toposortList(g):
  ...
  return pos             # retorna a lista de nodos em ordem topologica
